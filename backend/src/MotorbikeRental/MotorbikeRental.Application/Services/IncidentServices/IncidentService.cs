...
namespace MotorbikeRental.Application.Services.IncidentServices
{
    public class IncidentService : IIncidentService
    {
        ...
        public async Task<IncidentDto> CreateIncident(IncidentCreateDto incidentCreateDto, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                RentalContract rentalContract = await unitOfWork.RentalContractRepository.GetById(incidentCreateDto.ContractId, cancellationToken) 
                    ?? throw new NotFoundException("Rental contract not found");

                Employee employee = await unitOfWork.EmployeeRepository.GetEmployeeBasicInfoById(incidentCreateDto.ReportedByEmployeeId.Value, cancellationToken) 
        ...
        public async Task<IncidentDto> UpdateBeforeComplete(IncidentUpdateBeforeCompleteDto incidentUpdateBeforeCompleteDto, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                Incident incident = await unitOfWork.IncidentRepository.GetIncidentByIdWithIncludes(incidentUpdateBeforeCompleteDto.IncidentId, true, cancellationToken) 
                    ?? throw new NotFoundException("Incident not found");
                RentalContract rentalContract = await unitOfWork.RentalContractRepository.GetContractById(incidentUpdateBeforeCompleteDto.ContractId, cancellationToken) 
                    ?? throw new NotFoundException("Rental contract not found");
                Employee employee = await unitOfWork.EmployeeRepository.GetEmployeeBasicInfoById(incidentUpdateBeforeCompleteDto.ReportedByEmployeeId.Value, cancellationToken) 
                    ?? throw new NotFoundException("Employee not found");
        ...
        {
            Incident incident = await unitOfWork.IncidentRepository.GetById(incidentCompleteDto.IncidentId, cancellationToken) 
                ?? throw new NotFoundException($"Incident with id {incidentCompleteDto.IncidentId} not found");
            RentalContract rentalContract = await unitOfWork.RentalContractRepository.GetContractById(incident.ContractId, cancellationToken) 
                ?? throw new NotFoundException("Rental contract not found");
        ...
        private async Task<IncidentDto> GetIncident(Expression<Func<Incident, bool>> expression, CancellationToken cancellation = default)
        {
            ...
            Employee employee = await unitOfWork.EmployeeRepository.GetEmployeeBasicInfoById(incident.ReportedByEmployeeId.Value, cancellation) 
                ?? throw new NotFoundException("Employee not found");
            Motorbike motorbike = await unitOfWork.MotorbikeRepository.GetByIdWithIncludes(incident.MotorbikeId.Value, cancellation) 
                ?? throw new NotFoundException("Motorbike not found");
        ...
        public async Task<PaginatedDataDto<IncidentListDto>> GetIncidentsByFilter(IncidentFilterDto incidentFilterDto, CancellationToken cancellationToken = default)
        {
            (IEnumerable<Incident> incidents, int totalCount) = await unitOfWork.IncidentRepository.GetIncidentsByFilter(
                    incidentFilterDto.PageNumber, 
                    incidentFilterDto.PageSize, 
                    incidentFilterDto.MotorbikeStatus, 
                    incidentFilterDto.FromDate, 
                    incidentFilterDto.ToDate, 
                    incidentFilterDto.IsResolved, 
                    cancellationToken);
        ...
        {
            Incident? incident = await unitOfWork.IncidentRepository.GetIncidentByIdWithIncludes(incidentId, true, cancellationToken) 
                ?? throw new NotFoundException("Incident not found");
