...
namespace MotorbikeRental.Application.Services.UserServices
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public async Task<PaginatedDataDto<EmployeeListDto>> GetEmployeeByFilter(EmployeeFilterDto filter, CancellationToken cancellation = default)
        {
            ...
            (IEnumerable<Employee> data, int totalCount) = await unitOfWork.EmployeeRepository.GetFilterData(employeeId == null ? null : int.Parse(employeeId), filter.Search, filter.PageNumber, filter.PageSize, filter.RoleId, filter.Status, cancellation);
