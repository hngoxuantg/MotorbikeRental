...
namespace MotorbikeRental.Application.Services.DiscountServices
{
    public class DiscountService : IDiscountService
    {
        ...
        {
            (IEnumerable<Discount> discount, int totalCount) = await unitOfWork.DiscountRepository.GetFilterData(filter.Search, filter.PageNumber, filter.PageSize, filter.CategoryId, filter.FilterStartDate, filter.FilterEndDate, filter.IsActive, cancellationToken);
        ...
        {
            return mapper.Map<DiscountDto>(await unitOfWork.DiscountRepository.GetDiscountById(id, false, cancellationToken) ?? throw new NotFoundException($"Discount with id {id} not found"));
        }
        ...
        {
            Discount? discount = await unitOfWork.DiscountRepository.GetById(id, cancellationToken) ?? throw new NotFoundException($"Discount with id {id} not found");
